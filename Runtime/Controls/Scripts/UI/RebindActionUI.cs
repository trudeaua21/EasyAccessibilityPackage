        ...
        public void ResetToDefault()
        {
            ...
            if (action.bindings[bindingIndex].isComposite)
            {
                ...
                for (var i = bindingIndex + 1; i < action.bindings.Count && action.bindings[i].isPartOfComposite; ++i)
                {
                    ...
                    // save the change to the binding override
                    BindingSaveLoader.SaveBinding(action, i, action.bindings[i].path);
                }
            }
            else
            {
                action.RemoveBindingOverride(bindingIndex);
                BindingSaveLoader.SaveBinding(action, bindingIndex, action.bindings[bindingIndex].path);
            }
            UpdateBindingDisplay();
        }
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        // Save the new state of the action to prefs
                        BindingSaveLoader.SaveBinding(action, bindingIndex, operation.selectedControl.path);
